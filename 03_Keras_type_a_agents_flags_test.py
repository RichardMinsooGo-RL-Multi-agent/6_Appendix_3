...
class DQN_agnt_0:
    def __init__(self, state_size, action_size):
        ...
        if load_model:
            self.model.load_weights(model_path + "/model_agnt_0.h5")
...
class DDQN_agnt_1:
    def __init__(self, state_size, action_size):
        ...
        if load_model:
            self.model.load_weights(model_path + "/model_agnt_1.h5")
...
class DDQN_agnt_2:
    def __init__(self, state_size, action_size):
        ...
        if load_model:
            self.model.load_weights(model_path + "/model_agnt_2.h5")
...
class Dueling_agnt_3:
    def __init__(self, state_size, action_size):
        ...
        if load_model:
            self.model.load_weights(model_path + "/model_agnt_3.h5")
    ...
    def build_model(self):
        ...
        lay_1 = Dense(units=self.hidden1,activation='relu',kernel_initializer='he_uniform',\
                  name='hidden_layer_0')(state)
        lay_2 = Dense(units=self.hidden2,activation='relu',kernel_initializer='he_uniform',\
                  name='hidden_layer_1')(lay_1)
...
def main():
    ...
    while time.time() - start_time < agnt_0.training_time:
        ...
        while not done and ep_step < agnt_0.ep_trial_step:
            ...
            agnt_0_row = int(agent_rows[0][0])
            agnt_1_row = int(agent_rows[0][1])
            agnt_2_row = int(agent_rows[0][2])
            agnt_3_row = int(agent_rows[0][3])

            agnt_0_col = int(agent_cols[0][0])
            agnt_1_col = int(agent_cols[0][1])
            agnt_2_col = int(agent_cols[0][2])
            agnt_3_col = int(agent_cols[0][3])
            ...
            next_state_t = next_state_t.flatten()
            ...
            for ag_idx in range(4):
                temp_dis = np.abs(flag_rows[0][0]-agent_rows[0][ag_idx]) \
                        + np.abs(flag_cols[0][0]-agent_cols[0][ag_idx])
                distance_0[0][ag_idx] = temp_dis

            for ag_idx in range(4):
                temp_dis = np.abs(flag_rows[0][1]-agent_rows[0][ag_idx]) \
                        + np.abs(flag_cols[0][1]-agent_cols[0][ag_idx])
                distance_1[0][ag_idx] = temp_dis

            for ag_idx in range(4):
                temp_dis = np.abs(flag_rows[0][2]-agent_rows[0][ag_idx]) \
                        + np.abs(flag_cols[0][2]-agent_cols[0][ag_idx])
                distance_2[0][ag_idx] = temp_dis

            for ag_idx in range(4):
                temp_dis = np.abs(flag_rows[0][3]-agent_rows[0][ag_idx]) \
                        + np.abs(flag_cols[0][3]-agent_cols[0][ag_idx])
                distance_3[0][ag_idx] = temp_dis
            
            game_dist = np.min(distance_0) + np.min(distance_1) + np.min(distance_2) + np.min(distance_3) 
            ...
            if flag_n_agent > 2:
                ...
                print(game_arr)
            """
            if two_agents != 0 or flag_n_2_agent != 0 or three_agents != 0 or flag_n_3_agent != 0:
                panelty = 2
            else:
                panelty = 0
            """
            ...
            if done:
                ...
                reward_0 = -1 - remain_flags - game_dist - panelty
                reward_1 = -1 - remain_flags - game_dist - panelty
                reward_2 = -1 - remain_flags - game_dist - panelty
                reward_3 = -1 - remain_flags - game_dist - panelty
    ...
    agnt_0.model.save_weights(model_path + "/model_agnt_0.h5")
    agnt_1.model.save_weights(model_path + "/model_agnt_1.h5")
    agnt_2.model.save_weights(model_path + "/model_agnt_2.h5")
    agnt_3.model.save_weights(model_path + "/model_agnt_3.h5")
