...
def main():
    ...
    if load_model:
        for idx in range(n_agents):
            agents[idx].model.load_weights(model_path + "/Model_dueling_{}.h5".format(idx))
        ...
        while not done and ep_step < agents[0].ep_trial_step:
            ...
            for idx in range(n_agents):
                ...
                if game_arr[agent_rows[idx]][agent_cols[idx]] == 3:
                    act_arr[idx] = 4
                if act_arr[idx] == 0:
                    if (agent_rows[idx]+1) < n_cells:
                        agent_rows[idx] += 1
                if act_arr[idx] == 1:
                    if (agent_rows[idx]-1) > 0:
                        agent_rows[idx] -= 1
                if act_arr[idx] == 2:
                    if (agent_cols[idx]-1) > 0:
                        agent_cols[idx] -= 1
                if act_arr[idx] == 3:
                    if (agent_cols[idx]+1) < n_cells:
                        agent_cols[idx] += 1
                        
                # print("Rows, Cols :",agent_rows[idx],agent_cols[idx])
            ...
            # distances[0] = np.zeros((1,n_agents))
            # distances[1] = np.zeros((1,n_agents))
            # distances[2] = np.zeros((1,n_agents))
            # distances[3] = np.zeros((1,n_agents))
            ...
            for idx in range(n_agents):
                if game_arr[flag_rows[idx]][flag_cols[idx]] == 1:
                    distances[1][idx] = np.abs(flag_rows[idx]-agent_rows[1]) \
                        + np.abs(flag_cols[idx]-agent_cols[1])
                else:
                    distances[1][idx] = n_cells + n_cells

            for idx in range(n_agents):
                if game_arr[flag_rows[idx]][flag_cols[idx]] == 1:
                    distances[2][idx] = np.abs(flag_rows[idx]-agent_rows[2]) \
                        + np.abs(flag_cols[idx]-agent_cols[2])
                else:
                    distances[2][idx] = n_cells + n_cells

            for idx in range(n_agents):
                if game_arr[flag_rows[idx]][flag_cols[idx]] == 1:
                    distances[3][idx] = np.abs(flag_rows[idx]-agent_rows[3]) \
                        + np.abs(flag_cols[idx]-agent_cols[3])
                else:
                    distances[3][idx] = n_cells + n_cells
            """
            ...
            if done:
                ...
                # reward_0 = -1 - np.min(distances[0]) - remain_flags - game_dist
                # reward_1 = -1 - np.min(distances[1]) - remain_flags - game_dist
                # reward_2 = -1 - np.min(distances[2]) - remain_flags - game_dist
                # reward_3 = -1 - np.min(distances[3]) - remain_flags - game_dist
            ...
            # tmp_states 에서 re_shape 후에 저장 할것
            # agents[0].append_sample(state, int(act_arr[0]), reward_0, state_1, done)
            # agents[1].append_sample(state_1, int(act_arr[1]), reward_1, state_2, done)
            # agents[2].append_sample(state_2, int(act_arr[2]), reward_2, state_3, done)
            # agents[3].append_sample(state_3, int(act_arr[3]), reward_3, next_state, done)
            
            # agents[0].append_sample(state, int(act_arr[0]), reward_0, next_state, done)
            # agents[1].append_sample(state_1, int(act_arr[1]), reward_1, next_state, done)
            # agents[2].append_sample(state_2, int(act_arr[2]), reward_2, next_state, done)
            
            # print("Appended at Memory!!")
            # sys.exit()
    ...
    for idx in range(n_agents):
        agents[idx].model.save_weights(model_path + "/Model_dueling_{}.h5".format(idx))
